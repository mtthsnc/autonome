/**
 * Autonome plugin for OpenCode.ai
 *
...
export const AutonomePlugin = async ({ client, directory }) => {
    const homeDir = os.homedir();
    const projectSkillsDir = path.join(directory, '.opencode/skills');
    // Derive autonome skills dir from plugin location (works for both symlinked and local installs)
    const autonomeSkillsDir = path.resolve(__dirname, '../../skills');
    const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

    // Helper to generate bootstrap content
    const getBootstrapContent = (compact = false) => {
        const usingAutonomePath = skillsCore.resolveSkillPath('use-autonome', autonomeSkillsDir, personalSkillsDir);
        if (!usingAutonomePath) return null;

        const fullContent = fs.readFileSync(usingAutonomePath.skillFile, 'utf8');
...
**Skills naming (priority order):** project: > personal > autonome:`
...
- Autonome skills: \`autonome:skill-name\`
- Project skills override personal, which override autonome when names match`;

        return `<EXTREMELY_IMPORTANT>
You are using Autonome.

**IMPORTANT: The use-autonome skill content is included below. It is ALREADY LOADED - you are currently following it. Do NOT use the use_skill tool to load "use-autonome" - that would be redundant. Use use_skill only for OTHER skills.**
    ...
    return {
        tool: {
            use_skill: tool({
                ...
                args: {
                    skill_name: tool.schema.string().describe('Name of the skill to load (e.g., "autonome:use-brainstorm", "my-custom-skill", or "project:my-skill")')
                },
                execute: async (args, context) => {
                    const { skill_name } = args;

                    // Resolve with priority: project > personal > autonome
                    ...
                    if (forceProject || !skill_name.startsWith('autonome:')) {
                    ...
                    // Fall back to personal/autonome resolution
                    if (!resolved && !forceProject) {
                        resolved = skillsCore.resolveSkillPath(skill_name, autonomeSkillsDir, personalSkillsDir);
                    }
...
# ============================================`;
            ...
            find_skills: tool({
                description: 'List all available skills in the project, personal, and autonome skill libraries.',
                args: {},
                execute: async (args, context) => {
                    ...
                    const autonomeSkills = skillsCore.findSkillsInDir(autonomeSkillsDir, 'autonome', 3);

                    // Priority: project > personal > autonome
                    const allSkills = [...projectSkills, ...personalSkills, ...autonomeSkills];

                    if (allSkills.length === 0) {
                        return 'No skills found. Install autonome skills to ~/.config/opencode/autonome/skills/ or add project skills to .opencode/skills/';
                    }
