#!/bin/bash
# Run a use-subagents-dispatch test
# Usage: ./run-test.sh <test-name> [--plugin-dir <path>]
#
# Example:
#   ./run-test.sh react-todo --plugin-dir /path/to/autonome
...
TIMESTAMP=$(date +%s)
OUTPUT_BASE="/tmp/autonome-tests/$TIMESTAMP/use-subagents-dispatch"
...
PROMPT="Execute this plan using autonome:use-subagents-dispatch. The plan is at: $PLAN_PATH"
