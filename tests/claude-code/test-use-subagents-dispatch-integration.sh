#!/usr/bin/env bash
# Integration Test: use-subagents-dispatch workflow
...
echo "========================================"
echo " Integration Test: use-subagents-dispatch"
echo "========================================"
echo ""
...
This is a minimal plan to test the use-subagents-dispatch workflow.
...
# Run Claude with use-subagents-dispatch
# Capture full output to analyze
OUTPUT_FILE="$TEST_PROJECT/claude-output.txt"

# Create prompt file
cat > "$TEST_PROJECT/prompt.txt" <<'EOF'
I want you to execute the implementation plan at docs/plans/implementation-plan.md using the use-subagents-dispatch skill.
...
# IMPORTANT: Run from autonome directory so local dev skills are available
PROMPT="Change to directory $TEST_PROJECT and then execute the implementation plan at docs/plans/implementation-plan.md using the use-subagents-dispatch skill.
...
# Use timeout if available (Linux/coreutils), otherwise run without it
...
cd "$SCRIPT_DIR/../.." && $TIMEOUT_CMD claude -p "$PROMPT" --allowed-tools=all --add-dir "$TEST_PROJECT" --permission-mode bypassPermissions 2>&1 | tee "$OUTPUT_FILE" || {
    echo ""
    echo "================================================================================"
...
if grep -q '"name":"Skill".*"skill":"autonome:use-subagents-dispatch"' "$SESSION_FILE"; then
    echo "  [PASS] use-subagents-dispatch skill was invoked"
else
...
if [ $FAILED -eq 0 ]; then
    ...
    echo ""
    echo "The use-subagents-dispatch skill correctly:"
