...
if [ -z "$SKILL_NAME" ] || [ -z "$PROMPT_FILE" ]; then
    ...
    echo "Example: $0 use-debug ./test-prompts/debugging.txt"
    exit 1
fi

# Get the directory where this script lives (should be tests/skill-triggering)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Get the autonome plugin root (two levels up from tests/skill-triggering)
PLUGIN_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"

TIMESTAMP=$(date +%s)
OUTPUT_DIR="/tmp/autonome-tests/${TIMESTAMP}/skill-triggering/${SKILL_NAME}"
mkdir -p "$OUTPUT_DIR"
...
echo "Running claude with naive prompt..."
timeout 300 echo "$PROMPT" | claude \
