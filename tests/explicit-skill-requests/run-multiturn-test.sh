...
TIMESTAMP=$(date +%s)
OUTPUT_DIR="/tmp/autonome-tests/${TIMESTAMP}/explicit-skill-requests/multiturn"
mkdir -p "$OUTPUT_DIR"
...
# Turn 3: The critical test - ask for use-subagents-dispatch
echo ">>> Turn 3: Requesting use-subagents-dispatch..."
...
if grep -q '"name":"Skill"' "$TURN3_LOG" && grep -qE "$SKILL_PATTERN" "$TURN3_LOG"; then
    echo "PASS: Skill 'use-subagents-dispatch' was triggered in Turn 3"
    TRIGGERED=true
else
    echo "FAIL: Skill 'use-subagents-dispatch' was NOT triggered in Turn 3"
    TRIGGERED=false
fi
