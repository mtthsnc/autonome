#!/bin/bash
# Test where Claude explicitly describes use-subagents-dispatch before user requests it
...
TIMESTAMP=$(date +%s)
OUTPUT_DIR="/tmp/autonome-tests/${TIMESTAMP}/explicit-skill-requests/claude-describes"
mkdir -p "$OUTPUT_DIR"
...
claude -p "I have a plan at docs/plans/auth-system.md. Tell me about my options for executing it, including what use-subagents-dispatch means and how it works." \
    --model haiku \
...
echo ">>> Turn 2: Request use-subagents-dispatch..."
FINAL_LOG="$OUTPUT_DIR/turn2.json"
claude -p "use-subagents-dispatch, please" \
    --continue \
    --model haiku \
    --plugin-dir "$PLUGIN_DIR" \
    --dangerously-skip-permissions \
...
# Check final turn
SKILL_PATTERN='"skill":"([^"]*:)?use-subagents-dispatch"'
