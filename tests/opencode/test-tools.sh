...
if echo "$output" | grep -qi "autonome:use-brainstorm\|autonome:use-autonome\|Available skills"; then
    echo "  [PASS] find_skills tool brainstormed autonome skills"
else
...
# Test 3: Test use_skill with autonome: prefix
echo ""
echo "Test 3: Testing use_skill with autonome: prefix..."
echo "  Running opencode with autonome:use-brainstorm skill..."

output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load autonome:use-brainstorm and tell me the first few lines of what you received." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
# Check for expected content from brainstorm skill
if echo "$output" | grep -qi "brainstorm\|Launching skill\|skill.*loaded"; then
    echo "  [PASS] use_skill loaded autonome:use-brainstorm skill"
else
    echo "  [FAIL] use_skill did not load autonome:use-brainstorm correctly"
    echo "  Output was:"
    echo "$output" | head -50
    exit 1
fi
