...
# Create skills in personal and autonome locations for testing
mkdir -p "$TEST_HOME/personal-skills/shared-skill"
mkdir -p "$TEST_HOME/autonome-skills/shared-skill"
mkdir -p "$TEST_HOME/autonome-skills/unique-skill"
...
cat > "$TEST_HOME/autonome-skills/shared-skill/SKILL.md" <<'EOF'
---
name: shared-skill
description: Autonome version
---
# Autonome Shared
EOF

cat > "$TEST_HOME/autonome-skills/unique-skill/SKILL.md" <<'EOF'
---
name: unique-skill
description: Only in autonome
---
# Unique
EOF
...
function resolveSkillPath(skillName, autonomeDir, personalDir) {
    const forceAutonome = skillName.startsWith('autonome:');
    const actualSkillName = forceAutonome ? skillName.replace(/^autonome:/, '') : skillName;

    if (!forceAutonome && personalDir) {
    ...
    if (autonomeDir) {
        const autonomePath = path.join(autonomeDir, actualSkillName);
        const autonomeSkillFile = path.join(autonomePath, 'SKILL.md');
        if (fs.existsSync(autonomeSkillFile)) {
            return {
                skillFile: autonomeSkillFile,
                sourceType: 'autonome',
                skillPath: actualSkillName
            };
        }
...
const autonomeDir = '$TEST_HOME/autonome-skills';
const personalDir = '$TEST_HOME/personal-skills';

// Test 1: Shared skill should resolve to personal
const shared = resolveSkillPath('shared-skill', autonomeDir, personalDir);
console.log('SHARED:', JSON.stringify(shared));

// Test 2: autonome: prefix should force autonome
const forced = resolveSkillPath('autonome:shared-skill', autonomeDir, personalDir);
console.log('FORCED:', JSON.stringify(forced));

// Test 3: Unique skill should resolve to autonome
const unique = resolveSkillPath('unique-skill', autonomeDir, personalDir);
console.log('UNIQUE:', JSON.stringify(unique));

// Test 4: Non-existent skill
const notfound = resolveSkillPath('not-a-skill', autonomeDir, personalDir);
console.log('NOTFOUND:', JSON.stringify(notfound));
" 2>&1)

if echo "$result" | grep -q 'SHARED:.*"sourceType":"personal"'; then
    echo "  [PASS] Personal skills shadow autonome skills"
else
...
if echo "$result" | grep -q 'FORCED:.*"sourceType":"autonome"'; then
    echo "  [PASS] autonome: prefix forces autonome resolution"
else
    echo "  [FAIL] autonome: prefix not working"
    exit 1
fi

if echo "$result" | grep -q 'UNIQUE:.*"sourceType":"autonome"'; then
    echo "  [PASS] Unique autonome skills are found"
else
    echo "  [FAIL] Unique autonome skills not found"
    exit 1
fi
