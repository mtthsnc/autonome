...
# Verifies that skills are resolved with correct priority: project > personal > autonome
...
# 1. Create in autonome location (lowest priority)
mkdir -p "$HOME/.config/opencode/autonome/skills/priority-test"
cat > "$HOME/.config/opencode/autonome/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Autonome version of priority test skill
---
# Priority Test Skill (Autonome Version)

This is the AUTONOME version of the priority test skill.

PRIORITY_MARKER_AUTONOME_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/autonome/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Autonome version exists"
else
    echo "  [FAIL] Autonome version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides autonome
echo ""
echo "Test 2: Testing personal > autonome priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides autonome)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_AUTONOME_VERSION"; then
    echo "  [FAIL] Autonome version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|autonome" | head -10
fi

# Test 3: Test that project overrides both personal and autonome
echo ""
echo "Test 3: Testing project > personal > autonome priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Autonome version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit autonome: prefix bypasses priority
echo ""
echo "Test 4: Testing autonome: prefix forces autonome version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load autonome:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_AUTONOME_VERSION"; then
    echo "  [PASS] autonome: prefix correctly forces autonome version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] autonome: prefix did not force autonome version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
